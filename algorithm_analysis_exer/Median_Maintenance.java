...
class Maxheap{
	...
	public int n = 0; // the real length of the heap
	public int child, parent, child1, child2;

	public void insert(int value){
		heap[n] = value; // insert the new value in the last position of the arrayyy
		parent = (n+1)/2 - 1;
		// if the only node present is the root, then no need to bubble up
		if(parent < 0){
			n++; return;
		} 
		child = n;
	...
	public int del(){
		...
		n--;
		return top; // return the max element that was deleted from the max heap(the root)
	}
...
class Minheap {
	...
	public int del(){
		...
		n--;
		return top; // return the root(or min) element of the heap
	}
...
public class Median_Maintenance {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		String line;
		int count = 0; // count the rank of the number
		...
		while((line = br.readLine()) != null){
			...
			// get the median value from the previous insertion which is 
			//either the top of max heap or min heap, the unbalanced one
			// is alway in Minheap
			int maxno = maxh.n;
			...
			if(medianno == maxno){
				...
				median = minh.getMin();// since minh.n - max.n =1 or 0
			}
